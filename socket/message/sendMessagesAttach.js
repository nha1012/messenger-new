...
let sendMessagesAttach=(io)=>{
  ...
  io.on('connection', function (socket) {
    ...
    socket.on('client-add-new-attach-message', function ( contactFromClient){
      if(contactFromClient.conversationType!='group'){
        ...
        clients[contactFromClient.receiverId].forEach(element => {
          io.to(element).emit('server-add-new-attach-message', contactFromClient);
        });
      }
      }else{      
          ...
          result.member.forEach(user=>{
              if(user.userId!=socket.request.user._id){
                if(clients[user.userId]){
                  ...
                  io.sockets.connected[element].emit('server-add-new-attach-message', contactFromClient);
                })
                }               
              }              
          })
...
module.exports= sendMessagesAttach