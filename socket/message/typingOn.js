...
let typingOn=(io)=>{
  ...
  io.on('connection', function (socket) {
    ...
    socket.on('client-typing-on', function (contactFromClient){
      if(contactFromClient.isGroup==false){
        ...
        clients[contactFromClient.targetId].forEach(element => {
          io.to(element).emit('server-typing-on',socket.request.user._id);
        });
      }
      }else{      
          ...
          result.member.forEach(user=>{
              if(user.userId!=socket.request.user._id){
                if(clients[user.userId]){
                  ...
                  io.sockets.connected[element].emit('server-typing-on',contactFromClient.targetId);
                })
                }
              }  
          })
